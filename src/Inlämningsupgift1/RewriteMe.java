...
public class RewriteMe {
    ...
    public int getAmountOfQuestionsForACertainCategory(Category category) {
        return (int) questions.stream().filter(l -> l.getCategory().equals(category)).count();
    ...
    public List<String> getAllQuestionStringsBelongingACategory(Category category) {
        return questions.stream().filter(l -> l.getCategory().equals(category)).map(Question::getQuestionString).toList();
    }
    ...
    public List<String> getAllAnswerOptionsDistinct() {
        return questions.stream().map(Question::getAllAnswers).flatMap(Collection::stream).distinct().toList();
    }
    ...
    public boolean isThisAnAnswerOption(String answerCandidate) {
        return questions.stream().anyMatch(l -> l.getAllAnswers().contains(answerCandidate));
    ...
    public int getAnswerCandidateFrequncy(String answerCandidate) {
        return (int) questions.stream().filter(l -> l.getAllAnswers().contains(answerCandidate)).count();
